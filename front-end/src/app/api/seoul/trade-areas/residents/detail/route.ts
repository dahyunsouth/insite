...
// Resident population by quarter per trade area
const SERVICE = "VwsmTrdarRepopQq";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalResident = Number(found?.TOT_REPOP_CO ?? 0);
    const male = Number(found?.ML_REPOP_CO ?? 0);
    const female = Number(found?.FML_REPOP_CO ?? 0);
    const householdsTotal = Number(found?.TOT_HSHLD_CO ?? 0);
    const aptHouseholds = Number(found?.APT_HSHLD_CO ?? 0);
    const nonAptHouseholds = Number(found?.NON_APT_HSHLD_CO ?? 0);

    const age10 = Number(found?.AGRDE_10_REPOP_CO ?? 0);
    const age20 = Number(found?.AGRDE_20_REPOP_CO ?? 0);
    const age30 = Number(found?.AGRDE_30_REPOP_CO ?? 0);
    const age40 = Number(found?.AGRDE_40_REPOP_CO ?? 0);
    const age50 = Number(found?.AGRDE_50_REPOP_CO ?? 0);
    const age60 = Number(found?.AGRDE_60_ABOVE_REPOP_CO ?? 0);

    const maleRate = totalResident > 0 ? (male / totalResident) * 100 : 0;
    const femaleRate = totalResident > 0 ? (female / totalResident) * 100 : 0;
    const aptRate = householdsTotal > 0 ? (aptHouseholds / householdsTotal) * 100 : 0;
    const nonAptRate = householdsTotal > 0 ? (nonAptHouseholds / householdsTotal) * 100 : 0;
    const avgHouseholdSize = householdsTotal > 0 ? totalResident / householdsTotal : 0;
    ...
    const topAgeRate = totalResident > 0 ? (topAge.value / totalResident) * 100 : 0;
