...
export const useComparisonStore = create<ComparisonState>((set, get) => ({
  ...
  addToComparison: (trdarCd: string, trdarCdNm: string) => {
    ...
    if (exists) {
      logger.info('비교함에 이미 존재하는 상권입니다:', trdarCdNm);
      return;
    }
    
    // 최대 2개까지만 추가 가능
    if (comparisonTray.length >= 2) {
      logger.info('비교함이 가득참 - 최대 2개까지만 담을 수 있습니다. 시도한 상권:', trdarCdNm);
      return;
    }
    
    logger.info('비교함에 상권 추가 성공:', trdarCdNm);
    set(state => ({
  ...
  removeFromComparison: (trdarCd: string) => {
    ...
    if (removedItem) {
      logger.info('비교함에서 상권 제거:', removedItem.trdarCdNm);
    }
