...
export default function NowAddress({ onAddressClick, onAddressChange }: NowAddressProps) {
    ...
    if (status === (window as any).kakao.maps.services.Status.OK) {
      ...
      let dong = '';
        ...
        for (let i = 0; i < result.length; i++) {
          logger.info(`📋 결과 ${i}:`, result[i]);
          logger.info(`  - region_type: ${result[i].region_type}`);
          logger.info(`  - address_name: "${result[i].address_name}"`);
          logger.info(`  - region_1depth_name: "${result[i].region_1depth_name}"`);
          logger.info(`  - region_2depth_name: "${result[i].region_2depth_name}"`);
          logger.info(`  - region_3depth_name: "${result[i].region_3depth_name}"`);
          ...
          if (result[i].region_type === 'B' && result[i].region_2depth_name && result[i].region_2depth_name.includes('구')) {
            ...
            logger.info(`🏢 자치구 발견 (region_2depth): ${gu}`);
          }
          ...
          if (result[i].region_type === 'H' && result[i].region_3depth_name) {
            ...
            logger.info(`🏠 행정동 발견 (region_3depth): ${dong}`);
          } else if (result[i].region_type === 'B' && result[i].region_3depth_name && !dong) {
            // 법정동은 행정동이 없을 때만 사용 (예: 역삼동)
            dong = result[i].region_3depth_name;
            logger.info(`🏘️ 법정동 발견 (region_3depth): ${dong}`);
          } else if (result[i].region_type === 'L' && result[i].region_3depth_name && !dong) {
            // 리는 마지막 옵션
            dong = result[i].region_3depth_name;
            logger.info(`🌳 리 발견 (region_3depth): ${dong}`);
          }
        }
      
      logger.info(`📍 최종 주소: ${gu} ${dong}`);
      setCurrentAddress({ gu, dong });
      setIsLoading(false);
      
      // 주소 변경 콜백 호출
      if (onAddressChange && gu && dong) {
        logger.info(`📞 주소 변경 콜백 호출: ${gu} ${dong}`);
        logger.info(`🔗 onAddressChange 콜백 존재:`, !!onAddressChange);
        logger.info(`📝 전달할 파라미터 검증:`, {
          district: gu,
          dong: dong,
        ...
        logger.info(`✅ 주소 변경 콜백 호출 완료`);
      } else {
        logger.info(`❌ 주소 변경 콜백 호출 안함:`, { 
      ...
      logger.error('❌ Geocoder 실패:', status);
  ...
  useEffect(() => {
    if (!map || !window.kakao) {
      logger.info('❌ 지도 또는 카카오맵이 준비되지 않음');
      return;
    }

    // 초기 주소 캐시 적용 (즉시 표시)
    logger.info('🚀 초기 주소 캐시 적용:', INITIAL_ADDRESS_CACHE);
    if (onAddressChange) {
    ...
    const checkServicesAndStart = () => {
      ...
      logger.info('🗺️ 카카오맵 services 준비 완료, 주소 검색 시작');

      // 현재 지도 중심좌표로 주소를 검색해서 표시합니다 (가이드 코드와 동일)
      const initialCenter = map.getCenter();
      logger.info('📍 초기 중심 좌표:', initialCenter.getLat(), initialCenter.getLng());
          ...
          <div 
            ...
            onClick={() => {
              if (currentAddress.gu && currentAddress.dong && onAddressClick) {
                logger.info('🖱️ 주소 클릭:', currentAddress.gu, currentAddress.dong);
