...
export default function TradeAreaPicker({ title, value, onChange, accentColor, backgroundColor }: TradeAreaPickerProps) {
  ...
  const selectedSignguName = value.signguCode ? SIGNGU_NAME_BY_CODE.get(value.signguCode) : undefined;
  ...
  const currentOptions: PickerOption[] = useMemo(() => {
    ...
    if (step === 1) {
      const filtered = keyword
        ? SIGNGU_OPTIONS.filter((item) => normalize(`${item.name} ${item.code}`).includes(keyword))
        : SIGNGU_OPTIONS;
      return filtered.slice(0, 8).map((item) => ({ code: item.code, name: item.name }));
    }

    if (step === 2 && value.signguCode) {
      const base = ADSTRD_OPTIONS.filter((item) => item.signguCode === value.signguCode);
      const filtered = keyword
        ? base.filter((item) => normalize(`${item.name} ${item.code}`).includes(keyword))
        : base;
      return filtered.slice(0, 8).map((item) => ({ code: item.code, name: item.name }));
    }

    if (step === 3 && value.signguCode && value.adstrdCode) {
      const base = TRADE_AREA_OPTIONS.filter(
        (item) => item.signguCode === value.signguCode && item.adstrdCode === value.adstrdCode,
      );
      const filtered = keyword
        ? base.filter((item) => normalize(`${item.name} ${item.code} ${item.typeName}`).includes(keyword))
        : base;
    ...
    return [];
  }, [query, step, value.adstrdCode, value.signguCode]);
      ...
      ? createPortal(
          (
            <div
              ...
              className="z-50 max-h-56 overflow-y-auto rounded-xl border border-gray-200 bg-white shadow-lg"
              style={{
                position: "absolute",
